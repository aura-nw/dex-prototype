...
use auraswap::mock_querier::{mock_dependencies, WasmMockQuerier};
...
use auraswap::asset::{AssetInfo, PairInfo};
use auraswap::factory::{
    ConfigResponse, ExecuteMsg, InstantiateMsg, NativeTokenDecimalsResponse, QueryMsg,
};
use auraswap::pair::{InstantiateMsg as PairInstantiateMsg, MigrateMsg as PairMigrateMsg};
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_auraswap_factory(&[], &[("uusd".to_string(), 6u8)]);
    let asset_infos = [
...
fn create_pair_native_token_and_ibc_token() {
    ...
    deps = init(deps);
    deps.querier.with_auraswap_factory(
        &[],
...
fn reply_test() {
    ...
    // register auraswap pair querier
    deps.querier.with_auraswap_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {
